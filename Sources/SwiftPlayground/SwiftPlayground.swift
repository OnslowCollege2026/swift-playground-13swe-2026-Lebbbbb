...
// REDO FROM START SO FOLLOWS ALONG PROPERLY!!
...
import Foundation
    ...
    struct Rank: Comparable, CustomStringConvertible {
        ...
        static func from(amount: Double) -> Rank {
            if amount >= 250 { return Rank(label: "S Tier", level: 5) }
            if amount >= 100 { return Rank(label: "A Tier", level: 4) }
            ...
            return Rank(label: "F Tier", level: 0)
        }
    }
